#description: Adds user logoff script to OneDrive GPO to prevent logout hangs
#tags: GPO, OneDrive, User Logoff Script

<#
.SYNOPSIS
    Adds the OneDrive user logoff script to an existing OneDrive GPO.

.DESCRIPTION
    This script adds configure-onedrive-user-logoff.ps1 as a user logoff script to the
    existing "AVD - OneDrive & SharePoint Settings" GPO.
    
    The user logoff script runs BEFORE FSLogix profile unload and:
    - Stops OneDrive sync
    - Closes OneDrive gracefully
    - Releases file handles to prevent profile unmount hangs

.PARAMETER GPOName
    Name of the GPO. Default: "AVD - OneDrive & SharePoint Settings"

.PARAMETER ScriptPath
    Path to configure-onedrive-user-logoff.ps1 script. If not provided, searches common locations.
...
Write-Host "Adding OneDrive user logoff script to GPO..." -ForegroundColor Cyan
Write-Host "=" * 60
...
# Find the logoff script
if ([string]::IsNullOrEmpty($ScriptPath)) {
    $scriptName = "configure-onedrive-user-logoff.ps1"
...
$gpoSysvolPath = "\\$domain\SYSVOL\$domain\Policies\{$($gpo.Id)}\User\Scripts\Logoff"

# Create logoff scripts directory if it doesn't exist
if (-not (Test-Path $gpoSysvolPath)) {
    Write-Host "Creating logoff scripts directory..." -ForegroundColor Yellow
...
# Create/update scripts.ini file for user logoff scripts
...
try {
    ...
    if (-not $scriptRegistered) {
        ...
        # User logoff scripts.ini format:
        # [Logoff]
        # 0CmdLine=scriptname.ps1
        # 0Parameters=
        if ($scriptsIniContent -notmatch "\[Logoff\]") {
            $scriptsIniContent += "[Logoff]"
        }
        ...
        Write-Host "User logoff script registered in scripts.ini" -ForegroundColor Green
        Write-Host "Script will appear in 'Logoff' tab in GPO Editor (User Configuration)" -ForegroundColor Gray
    } else {
        Write-Host "User logoff script already registered in scripts.ini" -ForegroundColor Gray
    }
} catch {
    Write-Warning "Failed to create scripts.ini: $_"
    Write-Host "You may need to configure the logoff script manually in GPO Editor:" -ForegroundColor Yellow
    Write-Host "  User Configuration > Policies > Windows Settings > Scripts > Logoff" -ForegroundColor Gray
    Write-Host "  Click 'Add...' button and select: $gpoScriptPath" -ForegroundColor Gray
}

Write-Host ""
Write-Host "=" * 60
Write-Host "User Logoff Script Added Successfully" -ForegroundColor Green
Write-Host ""
...
Write-Host "  3. Have users log out to test" -ForegroundColor Gray
Write-Host ""
Write-Host "Note: This script runs BEFORE FSLogix profile unload, preventing logout hangs." -ForegroundColor Cyan
